data {
  ...
  array[N_subject] int<lower=1> N_repeat; // Number of observations per subject
  vector[N] y; // Data
  int<lower = 0, upper = 1> run; // Switch to evaluate the likelihood
}

transformed data {
  array[N_subject] int start; // Index of first observation for each subject
  array[N_subject] int end; // Index of last observation for each subject
...
generated quantities {
  array[N] real y_rep; // Replications
